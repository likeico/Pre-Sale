...
contract Likecoin is StandardToken {
  ...
  uint public INITIAL_SUPPLY = 100000000 * 10**8;  // 100 millions tokens
...
contract LikecoinICO is Pausable {
  using SafeMath for uint256;
  ...
  function LikecoinICO(uint256 _startTime, uint256 _endTime, uint256 _rate, address _wallet_address, address _token_address, uint256 _cap) {
  ...
  function buyTokens(address beneficiary) whenNotPaused validGasPrice private {
    ...
    Likecoin tok = Likecoin(token_address);
